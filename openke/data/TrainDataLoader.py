...
class TrainDataLoader(object):
	...
	def __init__(self, 
		...
		bern_flag = False, #？？
		filter_flag = True, #？？
		neg_ent = 1, #负样本实体数目（默认1）
		neg_rel = 0): #负样本关系数目（默认0）
		
		base_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../release/Base.so")) #？？？c++编写的内核还是啥？
		...
		self.cross_sampling_flag = 0 #是否交叉取样？
		self.read() #读取数据
	...
	def read(self):
		...
		self.relTotal = self.lib.getRelationTotal() #总关系
		self.entTotal = self.lib.getEntityTotal() #总实体
		self.tripleTotal = self.lib.getTrainTotal() #总三元组（训练数据）

		if self.batch_size == None: #批次大小为空的话，用总数整除批次次数
			self.batch_size = self.tripleTotal // self.nbatches
		if self.nbatches == None: #批次次数为空的话，用总数整除批次大小
